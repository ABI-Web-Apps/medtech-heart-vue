...
var showLVPInternal = function (chartName) {
  if (chartName != currentLVPName) {
    ...
    if (chartName == "CompensatedFailure" || chartName == "SmallInfarct") {
      ...
      chartName == "DecompensatedFailure" ||
      chartName == "LargeInfarct" ||
      chartName == "Arrhythmia"
    ) {
      colourName = "rgba(255,50,0,1)";
      widthvar = 3;
    }
    LVPchart.addSeries(chartName, currentLVP, {
      stroke: { color: colourName, width: widthvar },
    });
    if (
      chartName == "CompensatedFailure" ||
      chartName == "DecompensatedFailure" ||
      chartName == "SmallInfarct" ||
      chartName == "LargeInfarct" ||
      chartName == "Arrhythmia"
    ) {
...
var showECGInternal = function (chartName) {
  ...
  if (chartName != currentECGName) {
    ...
    console.log(minECGTime);
    console.log(maxECGTime);
    ...
    ECGchart.addSeries(chartName, currentECG, {
      stroke: { color: colourName, width: widthvar },
    });
    if (
      chartName == "CompensatedFailure" ||
