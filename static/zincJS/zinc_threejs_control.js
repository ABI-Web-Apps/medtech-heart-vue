...
function ZincCameraControls(object, domElement, renderer, scene) {
  var _this = this;
  var MODE = {
    NONE: -1,
    DEFAULT: 0,
  ...
  function onDocumentMouseDown(event) {
    ...
    if (rect === undefined) rect = _this.domElement.getBoundingClientRect();
    if (event.button == 0) {
  ...
  function onDocumentMouseUp(event) {
    ...
    if (zincRayCaster !== undefined) {
      if (
        _this.pointer_x_start == event.clientX - rect.left &&
        _this.pointer_y_start == event.clientY - rect.top
      ) {
  ...
  function onDocumentTouchStart(event) {
    ...
    if (len == 1) {
      ...
      _this.touchZoomDistanceEnd = _this.touchZoomDistanceStart = Math.sqrt(
        dx * dx + dy * dy
      );
    } else if (len == 3) {
  ...
  function onDocumentTouchEnd(event) {
    ...
    if (len == 1) {
      if (zincRayCaster !== undefined) {
        if (
          _this.pointer_x_start == event.touches[0].clientX - rect.left &&
          _this.pointer_y_start == event.touches[0].clientY - rect.top
        ) {
          zincRayCaster.pick(
            _this.cameraObject,
  ...
  function onDocumentWheelEvent(event) {
    ...
    var changes = 0;
    if (event.deltaY > 0) changes = _this.scrollRate;
    else if (event.deltaY < 0) changes = _this.scrollRate * -1;
  ...
  function translate() {
    if (typeof _this.cameraObject !== "undefined") {
      ...
      var distance = _this.cameraObject.position.distanceTo(
        _this.cameraObject.target
      );
      var fact = 0.0;
      if (
        _this.cameraObject.far > _this.cameraObject.near &&
        distance >= _this.cameraObject.near &&
      ...
      var old_near = new THREE.Vector3(
        _this.previous_pointer_x,
        height - _this.previous_pointer_y,
        0.0
      );
      var old_far = new THREE.Vector3(
        _this.previous_pointer_x,
        height - _this.previous_pointer_y,
        1.0
      );
      var new_near = new THREE.Vector3(
        _this.pointer_x,
        height - _this.pointer_y,
        0.0
      );
      var new_far = new THREE.Vector3(
        _this.pointer_x,
        height - _this.pointer_y,
        1.0
      );
      ...
      var dx =
        translate_rate *
        ((1.0 - fact) * (new_near.x - old_near.x) +
          fact * (new_far.x - old_far.x));
      var dy =
        translate_rate *
        ((1.0 - fact) * (new_near.y - old_near.y) +
          fact * (new_far.y - old_far.y));
      var dz =
        translate_rate *
        ((1.0 - fact) * (new_near.z - old_near.z) +
          fact * (new_far.z - old_far.z));
      _this.cameraObject.position.set(
        _this.cameraObject.position.x - dx,
        _this.cameraObject.position.y - dy,
        _this.cameraObject.position.z - dz
      );
      _this.updateDirectionalLight();
      _this.cameraObject.target.set(
        _this.cameraObject.target.x - dx,
        _this.cameraObject.target.y - dy,
        _this.cameraObject.target.z - dz
      );
    }
    _this.previous_pointer_x = _this.pointer_x;
    _this.previous_pointer_y = _this.pointer_y;
  ...
  this.rotateAboutLookAtpoint = function (a, angle) {
    ...
    var upa =
      a.x * _this.cameraObject.up.x +
      a.y * _this.cameraObject.up.y +
      a.z * _this.cameraObject.up.z;
    var upb =
      b.x * _this.cameraObject.up.x +
      b.y * _this.cameraObject.up.y +
      b.z * _this.cameraObject.up.z;
    var upc =
      c.x * _this.cameraObject.up.x +
      c.y * _this.cameraObject.up.y +
      c.z * _this.cameraObject.up.z;
    var cos_angle = Math.cos(angle);
    var sin_angle = Math.sin(angle);
    var new_b = new THREE.Vector3(
      cos_angle * b.x + sin_angle * c.x,
      cos_angle * b.y + sin_angle * c.y,
      cos_angle * b.z + sin_angle * c.z
    );
    var new_c = new THREE.Vector3(
      cos_angle * c.x - sin_angle * b.x,
      cos_angle * c.y - sin_angle * b.y,
      cos_angle * c.z - sin_angle * b.z
    );
    var eye_position = _this.cameraObject.target.clone();
    eye_position.x =
      eye_position.x + a.x * rel_eyea + new_b.x * rel_eyeb + new_c.x * rel_eyec;
    eye_position.y =
      eye_position.y + a.y * rel_eyea + new_b.y * rel_eyeb + new_c.y * rel_eyec;
    eye_position.z =
      eye_position.z + a.z * rel_eyea + new_b.z * rel_eyeb + new_c.z * rel_eyec;
    _this.cameraObject.position.set(
      eye_position.x,
      eye_position.y,
      eye_position.z
    ...
    _this.updateDirectionalLight();
    _this.cameraObject.up.set(
      a.x * upa + new_b.x * upb + new_c.x * upc,
    ...
    if (typeof _this.cameraObject !== "undefined") {
      ...
      if (0 < width && 0 < height) {
        ...
        if (tangent_dist > 0) {
          ...
          var d =
            dx * (_this.pointer_x - 0.5 * (width - 1)) +
            dy * (0.5 * (height - 1) - _this.pointer_y);
          if (d > radius) {
            d = radius;
          } else {
            if (d < -radius) {
          ...
          axis.set(
            Math.sin(phi) * a.x + Math.cos(phi) * e[0],
  ...
  function calculateZoomDelta() {
    ...
    if (_this._state === STATE.ZOOM) {
      ...
      delta =
        -1.0 * (_this.touchZoomDistanceEnd - _this.touchZoomDistanceStart);
  ...
  function flyZoom() {
    if (typeof _this.cameraObject !== "undefined") {
      ...
      if (dist + dy * dist > 0.01) {
        ...
        if (
          near_far_minimum_ratio * _this.cameraObject.far <
          ...
          if (_this.near_plane_fly_debt == 0.0) {
            var diff =
              _this.cameraObject.near -
              near_far_minimum_ratio * _this.cameraObject.far;
            _this.cameraObject.near =
              near_far_minimum_ratio * _this.cameraObject.far;
            _this.cameraObject.far -= diff;
            _this.near_plane_fly_debt -=
              near_far_minimum_ratio * _this.cameraObject.far;
          }
          _this.near_plane_fly_debt += dy * dist;
        }
  ...
  var updatePath = function (delta) {
    if (currentMode === MODE.PATH) {
      ...
      if (cameraPath) {
        ...
        var bot_pos = [
          cameraPath[bottom_frame * 3],
          cameraPath[bottom_frame * 3 + 1],
          cameraPath[bottom_frame * 3 + 2],
        ];
        var top_pos = [
          cameraPath[top_frame * 3],
          cameraPath[top_frame * 3 + 1],
          cameraPath[top_frame * 3 + 2],
        ];
        var current_positions = [];
        for (var i = 0; i < bot_pos.length; i++) {
          current_positions.push(
            proportion * bot_pos[i] + (1.0 - proportion) * top_pos[i]
          );
        }
        _this.cameraObject.position.set(
          current_positions[0],
        ...
        if (updateLightWithPathFlag) {
          ...
          _this.directionalLight.target.position.set(
            top_pos[0],
            top_pos[1],
            top_pos[2]
          );
  ...
  this.update = function (timeChanged) {
    ...
    if (controlEnabled) {
      ...
      if (_this._state === STATE.SCROLL) _this._state = STATE.NONE;
    }
  ...
  this.enableDeviceOrientation = function () {
    if (!deviceOrientationControl)
      deviceOrientationControl = new ModifiedDeviceOrientationControls(
        _this.cameraObject
      );
  };
  ...
  this.setDefaultCameraSettings = function (newViewport) {
    ...
    if (newViewport.farPlane) defaultViewport.farPlane = newViewport.farPlane;
    ...
    if (newViewport.upVector) defaultViewport.upVector = newViewport.upVector;
  };

  this.setCurrentCameraSettings = function (newViewport) {
    if (newViewport.nearPlane) _this.cameraObject.near = newViewport.nearPlane;
    if (newViewport.farPlane) _this.cameraObject.far = newViewport.farPlane;
    if (newViewport.eyePosition)
      _this.cameraObject.position.set(
  ...
  this.getViewportFromCentreAndRadius = function (
    centreX,
    centreY,
    centreZ,
    radius,
    ...
    var localEyePosition = [
      centreX + eyex * eye_distance,
      centreY + eyey * eye_distance,
    ...
    newViewport.upVector = [
      _this.cameraObject.up.x,
      _this.cameraObject.up.y,
  ...
  this.cameraTransition = function (
    startingViewport,
    endingViewport,
    durationIn
  ) {
    smoothCameraTransitionObject = new ZincSmoothCameraTransition(
      startingViewport,
      endingViewport,
      _this,
      durationIn
  ...
  this.autoTumble = function (
    tumbleDirectionIn,
    tumbleRateIn,
    stopOnCameraInputIn
  ) {
    cameraAutoTumbleObject = new ZincCameraAutoTumble(
      tumbleDirectionIn,
      tumbleRateIn,
      stopOnCameraInputIn,
      _this
  ...
  this.enableRaycaster = function (
    sceneIn,
    callbackFunctionIn,
    hoverCallbackFunctionIn
  ) {
    if (zincRayCaster == undefined)
      zincRayCaster = new ZincRayCaster(
        sceneIn,
        callbackFunctionIn,
        hoverCallbackFunctionIn,
        _this.renderer
...
function ZincSmoothCameraTransition(
  startingViewport,
  endingViewport,
  targetCameraIn,
  durationIn
  ...
  var completed = false;
  targetCamera.near = Math.min(
    startingViewport.nearPlane,
    endingViewport.nearPlane
  );
  targetCamera.far = Math.max(
    startingViewport.farPlane,
    endingViewport.farPlane
  );
  ...
  var updateCameraSettings = function () {
    ...
    var eyePosition = [
      startingEyePosition[0] * (1.0 - ratio) + endingEyePosition[0] * ratio,
      startingEyePosition[1] * (1.0 - ratio) + endingEyePosition[1] * ratio,
      startingEyePosition[2] * (1.0 - ratio) + endingEyePosition[2] * ratio,
    ];
    var targetPosition = [
      startingTargetPosition[0] * (1.0 - ratio) +
        endingTargetPosition[0] * ratio,
      startingTargetPosition[1] * (1.0 - ratio) +
        endingTargetPosition[1] * ratio,
      startingTargetPosition[2] * (1.0 - ratio) +
        endingTargetPosition[2] * ratio,
    ];
...
function ZincCameraAutoTumble(
  ...
  var computeTumbleAxisAngle = function (tumbleDirection) {
    var tangent_dist = Math.sqrt(
      tumbleDirection[0] * tumbleDirection[0] +
    ...
    var a = new THREE.Vector3(
      targetCamera.cameraObject.position.x - targetCamera.cameraObject.target.x,
    ...
    a.normalize();
    var b = new THREE.Vector3(
      targetCamera.cameraObject.up.x,
      targetCamera.cameraObject.up.y,
  ...
  update: (function () {
    ...
    return function update(camera) {
      ...
      if (needsUpdate) {
        ...
        var ymax =
          (near * Math.tan(THREE.Math.DEG2RAD * fov * 0.5)) / camera.zoom;
        var xmin, xmax;
...
function ModifiedDeviceOrientationControls(object) {
  var scope = this;
