...
Zinc.Glyph = function (geometry, materialIn, idIn) {
  ...
  this.getBoundingBox = function () {
    if (mesh) return new THREE.Box3().setFromObject(mesh);
    return undefined;
  };
  // 设置mesh的颜色， 并且将需要更新设置为true
...
Zinc.Glyphset = function () {
  ...
  // 对应了deforming_heart_metadata 中的 MorphColours
  ...
  console.log("hello:");
  // 假设这是metadata的组合8，9, 多半是组合8的内容
  this.load = function (glyphsetData, glyphURL, finishCallback) {
    // 将json8中的data属性全部加载进来
    console.log("hello:", glyphURL);
    ...
    else if (repeat_mode == "AXES_3D") numberOfVertices = numberOfVertices * 3;
  ...
  var resolve_glyph_axes = function (point, axis1, axis2, axis3, scale) {
    ...
    if (repeat_mode == "NONE" || repeat_mode == "MIRROR") {
      ...
      if (
        0.0 >
        final_axis3[0] *
          (final_axis1[1] * final_axis2[2] - final_axis1[2] * final_axis2[1]) +
          final_axis3[1] *
            (final_axis1[2] * final_axis2[0] -
              final_axis1[0] * final_axis2[2]) +
          final_axis3[2] *
            (final_axis1[0] * final_axis2[1] - final_axis1[1] * final_axis2[0])
      ) {
      ...
      if (repeat_mode == "MIRROR") {
        if (
          0.0 >
          mirrored_axis3[0] *
            (mirrored_axis1[1] * mirrored_axis2[2] -
              mirrored_axis1[2] * mirrored_axis2[1]) +
            mirrored_axis3[1] *
              (mirrored_axis1[2] * mirrored_axis2[0] -
                mirrored_axis1[0] * mirrored_axis2[2]) +
            mirrored_axis3[2] *
              (mirrored_axis1[0] * mirrored_axis2[1] -
      ...
      for (var k = 0; k < number_of_glyphs; k++) {
        ...
        final_axis3[0] =
          final_axis1[1] * use_axis2[2] - use_axis2[1] * final_axis1[2];
        final_axis3[1] =
          final_axis1[2] * use_axis2[0] - use_axis2[2] * final_axis1[0];
        final_axis3[2] =
          final_axis1[0] * use_axis2[1] - final_axis1[1] * use_axis2[0];
        ...
        final_axis2[0] =
          final_axis3[1] * final_axis1[2] - final_axis1[1] * final_axis3[2];
        final_axis2[1] =
          final_axis3[2] * final_axis1[0] - final_axis1[2] * final_axis3[0];
        final_axis2[2] =
          final_axis3[0] * final_axis1[1] - final_axis3[1] * final_axis1[0];
        magnitude = Math.sqrt(
          final_axis2[0] * final_axis2[0] +
            final_axis2[1] * final_axis2[1] +
            final_axis2[2] * final_axis2[2]
        );
        if (0.0 < magnitude) {
  ...
  var updateGlyphsetTransformation = function (
    current_positions,
    current_axis1s,
    current_axis2s,
    current_axis3s,
    ...
    if (repeat_mode == "AXES_2D" || repeat_mode == "MIRROR") numberOfGlyphs = 2;
    else if (repeat_mode == "AXES_3D") numberOfGlyphs = 3;
    ...
    for (var i = 0; i < numberOfPositions; i++) {
      ...
      var current_axis1 = [
        current_axis1s[current_index],
        current_axis1s[current_index + 1],
        current_axis1s[current_index + 2],
      ];
      var current_axis2 = [
        current_axis2s[current_index],
        current_axis2s[current_index + 1],
        current_axis2s[current_index + 2],
      ];
      var current_axis3 = [
        current_axis3s[current_index],
        current_axis3s[current_index + 1],
        current_axis3s[current_index + 2],
      ];
      var current_scale = [
        current_scales[current_index],
        current_scales[current_index + 1],
        current_scales[current_index + 2],
      ];
      var arrays = resolve_glyph_axes(
        current_position,
        current_axis1,
        current_axis2,
        current_axis3,
    ...
    if (morphVertices) {
      ...
      for (var i = 0; i < bottom_positions.length; i++) {
        current_positions.push(
          proportion * bottom_positions[i] +
            (1.0 - proportion) * top_positions[i]
        );
        current_axis1s.push(
          proportion * bottom_axis1[i] + (1.0 - proportion) * top_axis1[i]
        );
        current_axis2s.push(
          proportion * bottom_axis2[i] + (1.0 - proportion) * top_axis2[i]
        );
        current_axis3s.push(
          proportion * bottom_axis3[i] + (1.0 - proportion) * top_axis3[i]
        );
        current_scales.push(
          proportion * bottom_scale[i] + (1.0 - proportion) * top_scale[i]
        );
      }
    } else {
    ...
    if (colors != undefined) {
      if (morphColours) {
        ...
        for (var i = 0; i < bottom_colors.length; i++) {
          ...
          var resulting_color = new THREE.Color(
            bot.r * proportion + top.r * (1 - proportion),
  ...
  this.getBoundingBox = function () {
    ...
    for (var i = 0; i < glyphList.length; i++) {
      ...
      if (boundingBox1 == undefined) {
        ...
        // 将boundingBox2的数据合并到boundingBox1
  ...
  this.setMorphTime = function (time) {
    if (time > _this.duration) inbuildTime = _this.duration;
    else if (0 > time) inbuildTime = 0;
    else inbuildTime = time;
  ...
  this.render = function (delta, playAnimation) {
    ...
    if (playAnimation == true) {
      ...
      if (targetTime > _this.duration) targetTime = targetTime - _this.duration;
...
Zinc.Geometry = function () {
  ...
  this.setMorphTime = function (time) {
    if (_this.clipAction) {
      ...
      if (_this.clipAction.time < 0.0) _this.clipAction.time = 0.0;
      if (_this.timeEnabled == 1) _this.mixer.update(0.0);
    } else {
      if (time > _this.duration) inbuildTime = _this.duration;
      else if (0 > time) inbuildTime = 0;
      else inbuildTime = time;
  ...
  var morphColorsToVertexColors = function (targetGeometry, morph, clipAction) {
    if (
      ...
      targetGeometry.morphColors.length
    ) {
      var current_time = 0.0;
      if (clipAction)
        current_time =
          (clipAction.time / clipAction._clip.duration) *
          (targetGeometry.morphColors.length - 1);
      else
        current_time =
      ...
      for (var i = 0; i < targetGeometry.faces.length; i++) {
        var my_color1 = getColorsRGB(
          bottomColorMap.colors,
          targetGeometry.faces[i].a
        );
        var my_color2 = getColorsRGB(
          TopColorMap.colors,
          targetGeometry.faces[i].a
        );
        var resulting_color = [
          my_color1[0] * proportion + my_color2[0] * (1 - proportion),
          my_color1[1] * proportion + my_color2[1] * (1 - proportion),
          my_color1[2] * proportion + my_color2[2] * (1 - proportion),
        ];
        targetGeometry.faces[i].vertexColors[0].setRGB(
          resulting_color[0],
          resulting_color[1],
          resulting_color[2]
        );
        my_color1 = getColorsRGB(
          bottomColorMap.colors,
          targetGeometry.faces[i].b
        );
        ...
        targetGeometry.faces[i].vertexColors[1].setRGB(
          resulting_color[0],
          resulting_color[1],
          resulting_color[2]
        );
        ...
        targetGeometry.faces[i].vertexColors[2].setRGB(
          resulting_color[0],
          resulting_color[1],
          resulting_color[2]
        );
...
Zinc.Scene = function (containerIn, rendererIn) {
  ...
  setupCamera = function () {
    ...
    // 设置好 相机 灯光后， 交由zinc 来控制
    zincCameraControls = new ZincCameraControls(
      _this.camera,
      rendererIn.domElement,
      rendererIn,
      scene
    );
  ...
  nextAvailableInternalZincModelId = function () {
    var idFound = true;
    // console.log("inside nextAvailableModelId --> line:859:", zincGeometries);
    while (idFound == true) {
      startingId++;
      idFound = false;
  ...
  this.loadView = function (viewData) {
    // console.log("hello loadView, and viewData --> line:873", viewData);
    var viewPort = new ZincViewport();
    // console.log("zincviewpoint -->line: 876: ", viewPort);
  ...
  this.viewAllWithBoundingBox = function (boundingBox) {
    if (boundingBox) {
      ...
      var clip_factor = 4.0;
      var viewport = zincCameraControls.getViewportFromCentreAndRadius(
        centreX,
        centreY,
        centreZ,
        radius,
  ...
  var loadGlyphset = function (
    glyphsetData,
    glyphurl,
    groupName,
    finishCallback
  ) {
    console.log("hello:gly->line:968 ", glyphurl);
  ...
  var onLoadGlyphsetReady = function (
    xmlhttp,
    glyphurl,
    groupName,
    finishCallback
  ...
  this.loadGlyphsetURL = function (
    ...
    xmlhttp.onreadystatechange = onLoadGlyphsetReady(
      xmlhttp,
      glyphurl,
      groupName,
      finishCallback
  ...
  var loadMetaModel = function (
    ...
    // console.log("loader: -> 1051", loader);
    let test = "test: line 1155";
    loader.load(
      url,
      meshloader(
        modelId,
  ...
  var readMetadataItem = function (item, finishCallback) {
    // console.log("item-> 1069: ", item.Type);
    if (item) {
  ...
  //   console.log("hello loadSTL --> line 1092: ", url);
  //   num_inputs += 1;
  //   var modelId = nextAvailableInternalZincModelId();
  //   var colour = Zinc.defaultMaterialColor;
  //   var opacity = Zinc.defaultOpacity;
  ...
  this.loadMetadataURL = function (url, finishCallback) {
    // console.log("hello loadmeta url --> line 1134: ", url);
    var xmlhttp = new XMLHttpRequest();
  ...
  //   finishCallback
  // ) {
  //   console.log("hello loadmodelsurl url --> line 1157: ", url);
  //   var number = urls.length;
  //   num_inputs += number;
  //   for (var i = 0; i < number; i++) {
  //     var modelId = nextAvailableInternalZincModelId();
  //     var filename = urls[i];
  //     var loader = new THREE.JSONLoader(true); // three 0.79.0   99
  //     var colour = Zinc.defaultMaterialColor;
  //     var opacity = Zinc.defaultOpacity;
  ...
  this.loadViewURL = function (url) {
    // console.log("hello loadViewUrl --> line 1197: ", url);
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function () {
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
  ...
  //   console.log("hello loadFromViewURL --> line 1212: ", jsonFilePrefix);
  //   var xmlhttp = new XMLHttpRequest();
  //   xmlhttp.onreadystatechange = function () {
  //     if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
  //       var viewData = JSON.parse(xmlhttp.responseText);
  ...
  var addMeshToZincGeometry = function (
    mesh,
    modelId,
    localTimeEnabled,
    localMorphColour
  ) {
    // console.log("hello addMeshTOZincGeometry --> 1258");
  ...
  //   finishCallback
  // ) {
  //   console.log("hello objloader --> line 1293: ", modelId);
  //   return function (object) {
  //     num_inputs++;
  //     object.traverse(function (child) {
  //       if (child instanceof THREE.Mesh) {
  //         var zincGeometry = addMeshToZincGeometry(
  //           child,
  //           modelId,
  //           localTimeEnabled,
  //           localMorphColour
  //         );
  //         if (zincGeometry.morph) zincGeometry.morph.name = groupName;
  //         zincGeometry.groupName = groupName;
  //         if (
  //           finishCallback != undefined &&
  //           typeof finishCallback == "function"
  //         )
  ...
  this.addZincGeometry = function (
    ...
    // console.log("hello addZincGeometry --> line 1327: ", geometry);
    if (external == undefined) external = true;
    if (external) num_inputs++;
    isTransparent = false;
    if (1.0 > opacity) isTransparent = true;
    ...
    var newGeometry = addMeshToZincGeometry(
      mesh,
      modelId,
      localTimeEnabled,
      localMorphColour
  ...
  var meshloader = function (
    ...
    return function (geometry, materials) {
      // console.log("inside meshloader --> line 1388: ", geometry);
      // console.log(materials);
      var material = undefined;
      if (materials && materials[0]) {
        material = materials[0];
      }
      var zincGeometry = _this.addZincGeometry(
        geometry,
        modelId,
        colour,
        opacity,
  ...
  var allGlyphsetsReady = function () {
    // console.log(zincGlyphsets);
    for (var i = 0; i < zincGlyphsets.length; i++) {
      // console.log("zincGlyphset");
      zincGlyphset = zincGlyphsets[i];
      if (zincGlyphset.ready == false) return false;
    }
    return true;
  };
  ...
  this.renderGeometries = function (playRate, delta, playAnimation) {
    ...
    if (zincGeometries.length == num_inputs && allGlyphsetsReady()) {
      // console.log(zincGeometries.length);
      for (var i = 0; i < zincGeometries.length; i++) {
  ...
  this.render = function (renderer) {
    // console.log("render 1494");
    if (_this.autoClearFlag) renderer.clear();
    if (stereoEffectFlag && stereoEffect) {
      stereoEffect.render(scene, _this.camera);
      console.log("adasd");
    } else {
      // console.log("eles 1505-->");
...
Zinc.Renderer = function (containerIn, window) {
  ...
  // 在初始化中设置pc或者手机模式， 并且将render 赋值为 webgl render
  this.initialiseVisualisation = function () {
    ...
    if (onMobile) renderer = new THREE.WebGLRenderer({ antialias: false });
    else {
  ...
  loadView = function (viewData) {
    console.log("loadView render: line 1709:-> ");
  ...
  this.addZincGeometry = function (
    ...
    return currentScene.addZincGeometry(
      geometry,
      modelId,
      colour,
      opacity,
  ...
  this.addPreRenderCallbackFunction = function (callbackFunction) {
    ...
    preRenderCallbackFunctions[preRenderCallbackFunctions_id] =
      callbackFunction;
  ...
  this.render = function () {
    ...
    // console.log(additionalActiveScenes);
    // for (i = 0; i < additionalActiveScenes.length; i++) {
  ...
  this.transitionScene = function (endingScene, duration) {
    if (currentScene) {
      ...
      if (boundingBox) {
        ...
        var centreZ = (boundingBox.min.z + boundingBox.max.z) / 2.0;
        var clip_factor = 4.0;
        var endingViewport = currentCamera.getViewportFromCentreAndRadius(
          centreX,
          centreY,
          centreZ,
          radius,
        ...
        currentCamera.cameraTransition(
          startingViewport,
          endingViewport,
          duration
        );
